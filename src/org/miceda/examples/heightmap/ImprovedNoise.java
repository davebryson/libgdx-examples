...
public class ImprovedNoise {
    ...
    static public float[] heightData(int segmentSize){
        int width = segmentSize + 1;
        int size = width * width;
        final float[] data = new float[size];
        
        int quality = 1;
        float z = MathUtils.random() * 100;

        // Do several passes to get more detail
        for ( int iteration = 0; iteration < 4; iteration++ ) {
          for ( int i = 0; i < size; i ++ ) {
            float x = i % width;
            float y = (float)Math.floor( i / width );
            data[i] += Math.abs( noise( x / quality, y / quality, z ) * quality );
          }
          quality *= 5;
        }
